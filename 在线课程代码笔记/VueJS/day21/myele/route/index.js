...
module.exports = () => {
    ...
    function getDataFn(sqlStr, res) {
        db.query(sqlStr, (err, data) => {
            ...
            res.send({ status: 1, dataList: data })
        })
    }
    //用户列表接口
    route.get('/getuserlist', (req, res) => {
        //http://localhost:3000/api/getuserlist?pageIndex=1&pageSize=5
        //SELECT COUNT(*) FROM user 查询user表中总共有多少条数据
        //SELECT * FROM user LIMIT 0,5  //limit 限制查询 0开始索引  5查询几条数据
        //getDataFn(userListSql, res)
        console.log(req.query)
        let pageIndex = req.query.pageIndex
        let pageSize = req.query.pageSize
        let start = (pageIndex - 1) * pageSize
        let sqlUserData = `SELECT * FROM user LIMIT ${start},${pageSize}`
        let sqlUserTotal = `SELECT COUNT(*) FROM user`
        let userListDatas = []
        db.query(sqlUserTotal, (err, dataTotal) => {
            if (err) {
                console.log(err)
                res.status(500).send("database err").end();
            }
            let total = dataTotal[0]['COUNT(*)']
            console.log(total)
            userListDatas.push({ total: total })
            //res.send(data)
            db.query(sqlUserData, (err, data) => {
                if (err) {
                    console.log(err)
                    res.status(500).send("database err").end();
                }
                userListDatas.push(data)
                res.send({ status: 1, datalist: userListDatas })
            })
        })
    })
    //查看用户信息接口
